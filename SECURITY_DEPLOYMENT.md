...
安装更新后的依赖后，使用 `start.bat` 或 `python serve.py` 启动服务。它使用 Waitress，且只监听 `127.0.0.1:8000`；不要直接向公网开放 8000 端口。

`start-local.bat` 仅供本机 HTTP 开发使用。它会关闭 Secure Cookie，绝不能用于 Nginx 公网部署。

## 首次公网部署

1. 在本机通过 `start-local.bat` 登录后台。
2. 在“系统设置”中把公开访问基地址设置为完整 HTTPS 域名，例如 `https://images.example.com`。
3. 停止本地开发服务，使用 `start.bat` 启动安全模式。
4. 将 `nginx/picture-bed.conf.example` 中的域名和证书路径替换为真实值；将文件开头的两条 `limit_req_zone` 指令放到 Nginx 的 `http {}` 块中。
...
- 仅让运行 Picture Bed 的服务账号读写 `picture_bed.sqlite3` 与上传目录；备份文件也必须按同等敏感级别保护。
- Nginx 的站点根目录不要指向项目目录，全部请求应使用反向代理转发；否则数据库或其他运行文件可能被直接下载。
- 防火墙仅开放 80/443，拒绝外网访问 8000，以及数据库和上传目录所在主机的管理端口。
...
- Host 白名单、可信的一层 Nginx 转发头、安全响应头和登录/API 限速 Nginx 示例均已提供。
